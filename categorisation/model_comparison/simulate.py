...
def simulate_metalearner_with_fitted_coefficients(env=None, model_path=None, participant=0, beta=1., epsilon=0., method='soft_sigmoid', device='cpu', **kwargs):
    ...
    with torch.no_grad():
        ...
        if method == 'eps_greedy' or method == 'both': 
            ...
            model_choice_probs = model_choice_probs * (1 - epsilon) + epsilon * 0.5 
       

        # task measures
        model_choices = torch.distributions.Binomial(probs=model_choice_probs).sample()
        input_features = packed_inputs[..., :-1] # remove the last column which is the preivous choice
        prototypes = np.stack(stacked_prototypes) if env.return_prototype else torch.empty_like(input_features)
        stimulus_ids = stacked_stimulus_ids if stacked_stimulus_ids is not None else torch.empty_like(correct_choices) 
        ...
        for task_idx, (model_choices_task, correct_choices_task, sequence_lengths_task, prototypes_task, stimulus_ids_task, input_features_task) in enumerate(zip(model_choices, correct_choices, sequence_lengths, prototypes, stimulus_ids, input_features)):
            ...
            for trial_idx, (model_choice, correct_choice, stimulus_id, input_feature) in enumerate(zip(model_choices_task, correct_choices_task, stimulus_ids_task, input_features_task)):
                ...
                data = {**data,  **{f'prototype_feature{i+1}': prototypes_task[int(correct_choice)][i] for i in range(len(prototypes_task[0]))}} if env.return_prototype else data

                # make a pandas data frame
                df = pd.DataFrame(data, index=[0]) if task_idx==0 and trial_idx==0 else pd.concat([df, pd.DataFrame(data, index=[0])])
    ...
    print(f"model accuracy, beta, epsilon, method: {model_accuracy}, {beta}, {epsilon}, {method}")
...
def simulate_metalearner(args):
    ...
    def objective(env=None, model_name=None, method='soft_sigmoid', num_runs=1, **task_features):
        ...
        assert len(participants) == len(parameters), "number of participants and parameters must be equal"
    ...
    if args.task_name == 'badham2017':
        env = Badham2017() 
        task_features = {}
        df = objective(env=env, model_name=args.model_name, method=args.method, num_runs=1, **task_features)
    elif args.task_name == 'devraj2022':
        env = Devraj2022(return_prototype=True)
        df = objective(env=env, model_name=args.model_name, method=args.method, num_runs=1)
    else:
...
if __name__  == '__main__':
    ...
    parser.add_argument('--optimizer', choices=['differential_evolution', 'grid_search'], default='differential_evolution', help='method for computing model choice probabilities')
