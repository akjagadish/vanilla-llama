...
for task_id in task_ids: # iterate over all taskIDs that has passed some prelim checks
    ...
    try:
        ...
        X = SelectKBest(f_classif, k=3).fit_transform(X, targets)
        y = targets
...
sns.histplot(gini_coeff, ax=ax, bins=11, binrange=(0, bin_max), stat='probability', edgecolor='w', linewidth=1, color=COLORS['svm'])
