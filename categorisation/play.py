...
if __name__ == "__main__":
    ...
    #         r"x=\[(.*?)\][^\n]*?y=(\w)",
    #         r"x=\[([\d\.]+),\s*([\d\.]+),\s*([\d\.]+)\][^\n]*[y|&]=\s*(0|1)",
    #         r"x=\[?\s*([\d\.]+),\s*([\d\.]+),\s*([\d\.]+)\]?\s*(?:,|;|&|->| -> |---)?\s*[y|Y]\s*=\s*(0|1)",
    #         r"x=(\[.*?\])\s*---\s*y\s*=\s*([0-1])",
    #         r"x=(\[.*?\])\s*->\s*([0-1])",
    #         r"x=(\[.*?\]),\s*([0-1])",
    #         r'x=\[([-0-9., ]+)\]\s+→\s+y\s+=\s+(\d+)'
    ...
    #                     " The target, y, is a function of the input vector and can take on values of either y = A or y = B.\n\n"\
    #                     f" The following are {str(num_data)} input-target pairs generated for a classification problem using the template for each row:\n"\
    #                     "- [x1, x2, x3], y"
    
    # instructions = "In a classification problem, a collection of input-target pairs is provided. Each input, denoted as 'x,' is a vector with a length of " + str(num_dim) +\
    #                ". It consists of feature values (rounded to 2 decimals) that range continuously between 0 and 1, represented as [x1, x2, x3]. The target, denoted as 'y,' is determined based on the input vector and can be either 'A' or 'B'."\
    #                "The classification problem includes " + str(num_data) + " input-target pairs, which have been generated using a template. Each pair is represented in the following format:\n\n- [x1, x2, x3], y"


    # instructions = f"A classification problem consists of a list of input-target pairs."\
    #                f"Each input x is a vector of length 3, each entry takes continuous values between 0 and 1."\
    #                 f"The target y is a function of the input vector and takes values of either A or B.\n\n"\
    #                 "Please generate a list 100 input-target pairs using the following template for each row:"\
    #                 f"- [x1, x2, x3], y"

    # instructions = f"A classification problem consists of a set of input-target pairs."\
    #                     f" Each input, x, is a vector of length {str(num_dim)}, x = [x1, x2, x3], containing feature values that range continuously between -1 and 1."\
    #                     " The target, y, is a function of the input vector and can take on values of either y = 0 or y = 1.\n\n"\
    #                     f" The following are {str(num_data)} input-target pairs generated for one such classification problem:\n"\
    #                     "x=["
